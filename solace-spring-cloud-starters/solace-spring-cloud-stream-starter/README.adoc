...
Default: `native_only`
+
IMPORTANT: `headerTypeCompatibility` has been marked `@Deprecated` in v6.0.0 and will be removed in a future release. The default value has been changed from `serialize_and_encode_non_native_types` to `native_only`. After removal, the behavior will default to `native_only` (only native SMF headers will be supported).
...
+
IMPORTANT: `nonserializableHeaderConvertToString` has been marked `@Deprecated` in v6.0.0 and will be removed in a future release. This property will become irrelevant when `headerTypeCompatibility` is removed, as only native SMF headers will be supported at that point.
...
Default: `native_only`
+
IMPORTANT: `payloadTypeCompatibility` has been marked `@Deprecated` in v6.0.0 and will be removed in a future release. The default value has been changed from `serialize_non_native_types` to `native_only`. After removal, the behavior will default to `native_only` (only native SMF Payloads will be supported).
...
| solace_httpContentType
| String
| Read/Write
| The HTTP content type header value from interaction with an HTTP client.
...
[TIP]
====
To use a pre-existing queue by its exact name (e.g. one provisioned administratively), set `queueNameExpression` to a literal string expression:
[source,yaml]
----
spring.cloud.stream.solace.bindings.<bindingName>.consumer.queueNameExpression: "'my/existing/queue/name'"
----
====

[TIP]
====
To use the binding's `destination` value directly as the queue name, set `queueNameExpression` to `"destination"`:

[source,yaml]
----
spring.cloud.stream.bindings.<bindingName>.destination: "my/existing/queue/name"
spring.cloud.stream.solace.bindings.<bindingName>.consumer.queueNameExpression: "destination"
----
====

[TIP]
====
To apply this across all consumer bindings, set the default `queueNameExpression` instead of configuring it per binding:
[source,yaml]
----
spring.cloud.stream.bindings.<bindingName1>.destination: "queue1"
spring.cloud.stream.bindings.<bindingName2>.destination: "queue2"
spring.cloud.stream.solace.default.consumer.queueNameExpression: "destination"
----
====
...
[TIP]
====
To use a pre-existing queue by its exact name (e.g. one provisioned administratively), set `errorQueueNameExpression` to a literal string expression:
[source,yaml]
----
spring.cloud.stream.solace.bindings.<bindingName>.consumer.errorQueueNameExpression: "'my/existing/errorQueue/name'"
----
====
...
Consumer<Message<List<Payload>>> input() {
	return batchMsg -> { // <1>
		...
		List<Map<String, Object>> batchedHeaders = (List<Map<String, Object>>) batchMsg.getHeaders().get(BinderHeaders.BATCH_HEADERS); // <2>
...
<2> The `scst_batchHeaders` (i.e. BinderHeaders.BATCH_HEADERS) message header contains the consolidated list of message headers for each of the individual messages in the batch.
...
The producer binding will look for the `scst_batchHeaders` message header to determine if the supplied Spring message is either a batched Spring message or a regular Spring message.
...
[NOTE]
====
When using <<Batch Producers>>, the `SolaceBinderHeaders.CONFIRM_CORRELATION` header must be set at the root of the batched message, and **not** in the `BinderHeaders.BATCH_HEADERS` header.
