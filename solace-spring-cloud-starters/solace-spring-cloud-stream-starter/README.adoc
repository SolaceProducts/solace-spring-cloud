...
Note that applications can't differentiate between null payloads and empty payloads when consuming BytesMessage or XMLContentMessage from the wire. This is because Solace always converts empty payloads to null payloads when those message types are published.
