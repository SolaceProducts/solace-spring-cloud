...
+
When set to `queue`, the producer binds to a queue matching the `destination` name. All naming prefix and name generation options do not apply. However, the queue can be auto-provisioned with `provisionDurableQueue=true`.
+
Default: `topic`
