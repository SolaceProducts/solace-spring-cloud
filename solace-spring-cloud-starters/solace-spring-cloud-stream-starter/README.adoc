...
This header is only available when `Enable Client Delivery Count` is enabled on the queue. Note that a flow rebind may be necessary after `Enable Client Delivery Count` is turned on.
