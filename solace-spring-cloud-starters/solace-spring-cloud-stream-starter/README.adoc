...
Note that, while the Delivery Count feature is in controlled availability, `Enable Client Delivery Count` must be enabled on the queue and that a flow rebind is necessary after `Enable Client Delivery Count` is turned on.
