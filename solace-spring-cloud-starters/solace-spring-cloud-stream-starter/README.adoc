...
Note that, while the Delivery Count feature is in controlled availability, `Enable Client Delivery Count` must be enabled on the queue and a flow rebind or reconnect may be necessary after `Enable Client Delivery Count` is turned on.
