...
Refer to <<Failed Message Error Handling>> for more info.
...
An Error Queue can be provisioned for a particular consumer group by setting the `autoBindErrorQueue` consumer config option to `true`. This Error Queue is simply another durable queue which is named using a period-delimited concatenation of their consumer group name and `error`. And like the queues used for consumer groups, its endpoint properties can be configured by means of any consumer properties whose names begin with "errorQueue".
